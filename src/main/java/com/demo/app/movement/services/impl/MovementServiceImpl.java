...
public class MovementServiceImpl implements MovementService {
    ...
    public MovementServiceImpl(MovementRepository movementRepository, TransactionRepository transactionRepository, WebClient.Builder webClientPassive, @Value("${passive.card}") String pasiveCardUrl, @Value("${active.card}") String activeCardUrl, WebClient.Builder webClientActive, @Value("${user}") String userUrl, WebClient.Builder webClientUser) {
    ...
    private Mono<CurrentAccount> findCurrentAccountByIdentifier(String id) {
        return webClientPassiveCard.get().uri("/currentAccount/" + id).retrieve().bodyToMono(CurrentAccount.class);
    }

    private Mono<SavingAccount> findSavingAccountByIdentifier(String id) {
        return webClientPassiveCard.get().uri("/savingAccount/" + id).retrieve().bodyToMono(SavingAccount.class);
    }

    private Mono<FixedTermAccount> findFixedTermAccountByIdentifier(String id) {
        return webClientPassiveCard.get().uri("/fixedTermAccount/" + id).retrieve().bodyToMono(FixedTermAccount.class);
    }

    private Mono<CreditAccount> findProductById(String idProduct) {
        return webClientActiveCard.get().uri("/creditAccount/" + idProduct).retrieve().bodyToMono(CreditAccount.class);
    }

    private Mono<CreditAccount> findCreditAccountById(String id) {
        return webClientActiveCard.get().uri("/creditAccount/" + id).retrieve().bodyToMono(CreditAccount.class);
    }

    private Mono<CreditAccount> updateCreditAccount(CreditAccount creditAccount) {
        return webClientActiveCard.put().uri("/creditAccount/" + creditAccount.getId()).body(Mono.just(creditAccount), CreditAccount.class).retrieve().bodyToMono(CreditAccount.class);
    }
    ...
    private Mono<Movement> createMovementAndTransaction(Movement movement) {
        return movementRepository.save(movement).map(x -> {
            transactionRepository.save(createTransaction(x, movement.getType()))
                    .map(tr -> {
                        movement.setIdentifier(movement.getTargetIdentifier());
                        movement.setTargetIdentifier(tr.getIdentifier());
                        return transactionRepository.save(createTransaction(x, !movement.getType()));
                    });
            return x;
        });
    }
    ...
    public Mono<Movement> saveTransactionOfCreditAccount(Movement movement) {
        Mono<CreditAccount> account = findCreditAccountById(movement.getIdentifier()).flatMap(x -> {
            if (x.getCvc().equals(movement.getCvc())) {
                movement.setCommission(new BigDecimal(0));
                x.setBalance(x.getBalance().add(movement.getAmount().negate()));
